...
- The worker owns durable lifecycle sweeping: it sweeps once before the first job drain (including `RAS_WORKER_SINGLE_RUN=true`) and then no more often than `SUBSCRIPTION_LIFECYCLE_SWEEP_MS`. The default is 15 minutes; only integer millisecond values from 60,000 through 86,400,000 are accepted, with invalid values safely falling back to the default. Each sweep uses the worker server clock, catches/logs failures so job draining continues, and retries on the next cadence. This documents local runtime behavior only; it makes no deployment claim.
