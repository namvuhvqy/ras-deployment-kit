...
- [x] Zernio RCA received 2026-07-30: for API-originated Facebook/Instagram messages, Zernio previously deduplicated Meta outgoing echoes before emitting `message.sent`; Zernio fixed it in production and now emits `message.sent` exactly once from the send-path row. `message.delivered` remains an independent Meta event; ordering is not guaranteed because the Page is connected under two Zernio users. RAS dedupe remains rooted in `payload.id`.
