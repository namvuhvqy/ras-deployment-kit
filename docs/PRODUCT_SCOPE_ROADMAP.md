...
- [ ] **Production release gate:** prepare an isolated production release candidate: record current production backend artifact and frontend deployment IDs, verify production deployment transport/configuration, re-run `npm run check`, and keep unrelated dirty frontend payment work out of the release. Production mutation remains an explicit human approval gate.
