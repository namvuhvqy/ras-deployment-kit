...
- Before production: replace the JSON-backed fixed window with a shared token-bucket backend, add expiry/rotation UX, API-only entitlement enforcement, and a RAS MCP server that allow-lists tenant-safe tools. Do not expose Zernio's broad MCP surface directly.
