...
- [ ] **Production release gate (blocked, do not deploy yet):** obtain one *new* signed staging `message.sent` and `message.delivered` for one approved test reply after the Zernio provider fix. For each, verify provider delivery log shows HTTP `200`, RAS records the event/job idempotently, and durable `inboxMessages` has lifecycle mirrors with no second provider send. Earlier `message.delivered` entries at 2026-07-29 18:29/18:52 returned `422` before commit `8fcd968`; they are historical failures, not evidence against the deployed handler.
