...
async function withApi(state: Record<string, unknown>, run: (url: string, path: string) => Promise<void>) {
  ...
  const child = spawn(process.execPath, ['dist/apps/ras-api/src/server.js'], { cwd: process.cwd(), env: { ...process.env, PORT: String(port), RAS_DB_PATH: path }, stdio: ['ignore', 'pipe', 'pipe'] });
